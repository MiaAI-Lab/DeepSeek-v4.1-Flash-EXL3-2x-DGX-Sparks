...
// exl3_moe_x: the shipped exl3_moe kernel body with the pipeline shape lifted to template
// parameters so variants can be built side by side:
//   TK   K-tile (32 = shipped: 512 threads / 2 sub_k; 64 = 1024 threads / 4 sub_k)
//   SHS  cp.async shared-memory stages (shipped 3)
//   FRS  register fragment stages (shipped 3)
//   MINB __launch_bounds__ min blocks per SM (shipped 1; 2 caps registers at 64/thread)
template<int t_bits, int MOE_TILESIZE_N, int cb, int TK, int SHS, int FRS, int MINB>
__global__ __launch_bounds__(EXL3_GEMM_BASE_THREADS * TK / 16, MINB)
void exl3_moe_x_kernel(EXL3_MOE_KERNEL_ARGS)
{
    const int group_idx = blockIdx.z;
    const int block_idx = blockIdx.x;
    const int group_size = gridDim.x;  // SMs per expert, set at launch
    const int num_groups = gridDim.z;
    const int block_threads = EXL3_GEMM_BASE_THREADS * TK / 16;  // blockDim.x
